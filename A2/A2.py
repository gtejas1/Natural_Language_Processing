...
def main() :
    ...
    m1 = Sequential()
    m1.add(Embedding(vocab_len, embed_len, embeddings_initializer=Constant(embedding_matrix)))
    ...
    # Model 2: Non-Bidirectional
    m2 = Sequential()
    m2.add(Embedding(vocab_len, embed_len, embeddings_initializer=Constant(embedding_matrix)))
    m2.add(LSTM(10, return_sequences=True, activation="relu"))
    m2.add(TimeDistributed(Dense(50,activation="relu")))
    m2.add(Dense(tag_len,activation="softmax"))

    # Model 3: Using GRU instead of LSTM
    m3 = Sequential()
    m3.add(Embedding(vocab_len, embed_len, embeddings_initializer=Constant(embedding_matrix)))
    m3.add(GRU(10, return_sequences=True, activation="relu"))
    ...
    # evaluation
    evaluate_ie(te_sent, te_tag, te_predicted1, entity_types,"eval_own_m1.txt")
    evaluate_ie(te_sent, te_tag, te_predicted2, entity_types,"eval_own_m2.txt")
    evaluate_ie(te_sent, te_tag, te_predicted3, entity_types,"eval_own_m3.txt")
    ...
    # evaluation
    evaluate_ie(te_sent, te_tag, te_predicted, entity_types,"eval_own.txt")
